...
if (file_exists("$basePath/config/application.config.php")) {
    ...
    if (!isset($appConfig['modules']['Newage\Generators'])) {
        $appConfig['modules'][] = 'Newage\Generators';
        $appConfig['module_listener_options']['module_paths']['Newage\Generators'] = __DIR__;
    }
} else {
    $appConfig = array(
        'modules' => array(
